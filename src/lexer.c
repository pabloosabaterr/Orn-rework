...
static struct token error_token(struct lexer_context *lexer, const char *start, int len, int line,
...
static struct token token_id(struct lexer_context *lexer, const char *start)
{
    ...
    case 'o':
        if (len == 3 && !memcmp(start, "obj", 3))
            return create_token(TK_OBJ, start, len);
        break;
    case 'r':
...
static struct token token_number(struct lexer_context *lexer, const char *start)
{
    if (*start == '0' && !is_end(lexer)) {
        if (follow_next(lexer, 'x') || follow_next(lexer, 'X')) {
            ...
            return create_token(TK_HEX, start, (size_t)(lexer->current - start));
        }
        if (follow_next(lexer, 'o') || follow_next(lexer, 'O')) {
            const char *digits = lexer->current;
            while (!is_end(lexer) && *lexer->current >= '0' && *lexer->current <= '7')
                advance(lexer);
            if (lexer->current == digits)
                goto error_post_prefix;
            return create_token(TK_OCTAL, start, (size_t)(lexer->current - start));
        }
        if (follow_next(lexer, 'b') || follow_next(lexer, 'B')) {
            const char *digits = lexer->current;
            while (!is_end(lexer) && (*lexer->current == '0' || *lexer->current == '1'))
                advance(lexer);
            if (lexer->current == digits)
                goto error_post_prefix;
            return create_token(TK_BINARY, start, (size_t)(lexer->current - start));
        }
    }
...
error_post_prefix:
    return error_token(lexer, start, (int)(lexer->current - start), lexer->line, lexer->col - 2,
...
static struct token token_stringlit(struct lexer_context *lexer, const char *start)
{
    ...
    while (!is_end(lexer) && !is_next(lexer, '"')) {
        ...
        if (is_next(lexer, '\\')) {
            ...
            if (err)
                return error_token(lexer, start, (int)(lexer->current - start), start_line,
                                   start_col, err);
        } else {
            advance(lexer);
        }
    ...
    if (is_end(lexer))
        return error_token(lexer, start, (int)(lexer->current - start), start_line, start_col,
                           "unterminated string");
...
struct token lexer_get_next_token(struct lexer_context *lexer)
{
    ...
    case ':':
        if (follow_next(lexer, ':'))
            return create_token(TK_DECL, start, 2);
        if (follow_next(lexer, '='))
            return create_token(TK_WALRUS, start, 2);
    ...
    case '+':
        ...
        if (is_next(lexer, '+')) {
            advance(lexer);
            return error_token(lexer, start, 2, lexer->line, lexer->col - 2,
    ...
    default:
        ...
        return error_token(lexer, start, 1, lexer->line, lexer->col - 1, "unexpected character");
    }
}
...
int lexer_dump_tokens(struct lexer_context *lexer)
{
    ...
    while ((token = lexer_get_next_token(lexer)).type != TK_EOF) {
        ...
        printf("%.*s [%s] - %d:%d\n", (int)token.len, token.lex, token_type_str(token.type),
