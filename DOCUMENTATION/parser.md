...
# Pending stuff (post v1)

None of this is in the v1 grammar. These are ideas for when the
compiler can produce real binaries and actual programs can be written.

## v1.1 : if/match as expressions, null safety

Let if and match return values:
```
let x = if cond { 1 } else { 2 };
```
Both branches must have the same type.

Optional types so you don't depend on null pointers:
```
let p: ?*int = get_ptr();
must check for null before using p
```
...
Result and Optional go in early so the stdlib is built around them
from day one. No migration pain later.

## v1.2 — traits, generics, methods with self
...
```
fn swap<T>(a: *T, b: *T) { ... }
fn print_all<T: Printable>(items: *T, len: int) {
    for i in 0..len {
        items[i].print();
    }
}
...
Traits land together with generics so generic params can be
constrained. Without traits, generics would be like C++ templates,
no constraints and unreadable errors.
...
p.distance(&q) is sugar for Point::distance(&p, &q)
```

## v1.3 : closures, compiler attributes
